...
/datum/map_template/proc/preload_size()
	...
	for (var/mappath in mappaths)
		...
		if(M)
			...
			log_and_message_admins("could not preload [name]; no metadata")
			return FALSE
...
/datum/map_template/proc/init_atoms(var/list/atoms)
	...
	if(atoms.len)
		...
		log_and_message_admins("atoms.len is '[atoms.len]'?")

	if(cables.len)
		SSmachines.setup_powernets_for_cables(cables)
	if(atmos_machines.len)
...
/datum/map_template/proc/load(turf/T, centered=FALSE)
	...
	if(!T)
		log_and_message_admins("could not load [name]; no turf")
		return
	if(T.x+width > world.maxx)
		log_and_message_admins("could not load [name]; x+width ([T.x+width]) > world.maxx ([world.maxx])")
		return
	if(T.y+height > world.maxy)
		log_and_message_admins("could not load [name]; x+width ([T.x+height]) > world.maxx ([world.maxy])")
		return
	...
	for (var/mappath in mappaths)
		...
		if (M)
			...
			log_and_message_admins("could not load [name]; no metadata")
			return FALSE

	//initialize things that are normally initialized after map load
	init_atoms(atoms_to_initialise)
	init_shuttles(shuttle_state)
	for (var/turf/U in atoms_to_initialise)
		if (U.dynamic_lighting && U.loc:dynamic_lighting)
			U.lighting_build_overlay()

//	SSlighting.InitializeTurfs(atoms_to_initialise)	// Hopefully no turfs get placed on new coords by SSatoms.
	log_game("[name] loaded at at [T.x],[T.y],[T.z]")
	loaded++

	return atoms_to_initialise
